...
export let _slideUp = (target, duration = 800, showmore = 0) => {
...
export let _slideDown = (target, duration = 800, showmore = 0) => {
	if (!target.classList.contains('_slide')) {
		target.classList.add('_slide');
...
export let _slideToggle = (target, duration = 800) => {
	if (target.hidden) {
...
export function menuInit() {
	...
	const menuLinks = document.querySelectorAll('.menu__link > button');
	...
	if (menuButton) {
		...
		menuSubItems.forEach(subitem => {
			subitem.addEventListener("click", function (e) {
				if (e.target.closest('.menu__sublink > button')) {
					menuSubItem.forEach(item => {
						let elem_active = item.classList.contains("_active")
						menuSubItem.forEach(el => { el.classList.remove('_active'); });
						item.classList.toggle('_active', !elem_active)
					});
